...
/*	ASensorList sensor_list = nullptr;
	...
	SENSOR_REFRESH_PERIOD_US = int32_t(1e6 / fps_req);
...
//        LOGI("acc timestamp %f", *(static_cast<int64_t*>(acc_mem) + 2)/1e6);
...
//        LOGI("acc data %f, %f, %f", acc_data->x, acc_data->y, acc_data->z);
        ...
        if (accMsg.ts - last_acc_ts_ < 1e6) {// 1ms
...
//        LOGI("gyro timestamp %f", *(static_cast<int64_t*>(gyro_mem) + 2)/1e6);
...
//        LOGI("gyro data %f, %f, %f", gyro_data->x_uncalib, gyro_data->y_uncalib, gyro_data->z_uncalib);
        ...
        if (gyroMsg.ts - last_gyro_ts_ < 1e6) {

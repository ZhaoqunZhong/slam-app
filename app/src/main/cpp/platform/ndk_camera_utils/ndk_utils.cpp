...
void printCamProps(ACameraManager *cameraManager, const char *id)
{
  ...
  long long int minExposure = entry.data.i64[0];
  long long int maxExposure = entry.data.i64[1];
  LOGI("camProps: minExposure=%lld vs maxExposure=%lld", minExposure, maxExposure);
  ////////////////////////////////////////////////////////////////
  ...
  LOGI("camProps: minSensitivity=%d vs maxSensitivity=%d", minSensitivity, maxSensitivity);
  ////////////////////////////////////////////////////////////////
  ...
  for (int i = 0; i < entry.count; i += 4)
  {
    ...
    //if (format == System::img_format_)
    {
      int32_t width = entry.data.i32[i + 1];
      int32_t height = entry.data.i32[i + 2];
      LOGI("camProps: maxWidth=%d vs maxHeight=%d", width, height);
    }
  }
  ...
  LOGI("camProps: %d", orientation);

  //focus calibration
  ACameraMetadata_getConstEntry(metadataObj,
                                ACAMERA_LENS_INFO_FOCUS_DISTANCE_CALIBRATION, &entry);
  uint8_t focus_calibration = entry.data.u8[0];
  LOGI("Focus calibartion: %d", focus_calibration);

  //focus range
  ACameraMetadata_getConstEntry(metadataObj,
                                ACAMERA_LENS_FOCUS_RANGE, &entry);
  float min_focus = entry.data.f[0];
  float max_focus = entry.data.f[1];
  LOGI("Focus range: minFocus=%f vs maxFocus=%f", min_focus, max_focus);

/*  //min focus
  ACameraMetadata_getConstEntry(metadataObj,
                                ACAMERA_LENS_INFO_MINIMUM_FOCUS_DISTANCE, &entry);
  float minFocus = entry.data.f[0];
  LOGI("Minimum focus distance: %f", minFocus);

  //current focus
  ACameraMetadata_getConstEntry(metadataObj,
                                ACAMERA_LENS_FOCUS_DISTANCE, &entry);
  float focus = entry.data.f[0];
  LOGI("Current focus distance: %f", focus);*/
}
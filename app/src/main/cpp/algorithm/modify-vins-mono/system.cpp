...
namespace Initializer {
    ...
    System::System(uint image_height, uint image_width, double fx, double fy, double alpha_x,
                   double alpha_y,
                   ...
                   double gravity_norm, initial_result_callback cbk) : bStart_backend(true),
        ...
        if (type == RAD_TAN) {
            ...
            trackerData[0].m_camera = CameraFactory::instance()->generateCamera(
                    Camera::KANNALA_BRANDT, "",
        ...
        if (imageProcessCallback_) {
            auto f = [](uint64_t ts, cv::Mat mat, System *sys) {
                ...
                for (unsigned int j = 0; j < sys->trackerData[0].cur_pts.size(); j++) {
                    ...
                    cv::circle(show_img, sys->trackerData[0].cur_pts[j], 2,
...
/*            slam_processed_image image;
    ...
    void
    System::subImuData(double dStampSec, const Eigen::Vector3d &vGyr, const Eigen::Vector3d &vAcc) {
        if (estimator.initial_finished_ || !thread_initialized_)
            return;
...
// thread: visual-inertial odometry
    void System::process() {
        while (bStart_backend) {
            ...
            m_estimator.lock();
            LOG_IF(WARNING, measurements.size() > 1)
                            << "measurement size " << measurements.size() << " ";

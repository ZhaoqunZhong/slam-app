    ...
    void RefineGravityAndBias(map<double, ImageFrame> &all_image_frame, Vector3d &g, Vector3d &ba,
        ...
        for (int k = 0; k < 20; k++) {
            ...
            for (frame_i = all_image_frame.begin();
                ...
                frame_j = next(frame_i);
                TicToc tt;
                frame_j->second.pre_integration->repropagate(ba,
                                                             bg); // repropagate imu pre-integration with new biases
                ...
                Eigen::Matrix3d dp_dba = frame_j->second.pre_integration->jacobian.block<3, 3>(O_P,
                                                                                               O_BA);
                Eigen::Matrix3d dp_dbg = frame_j->second.pre_integration->jacobian.block<3, 3>(O_P,
                                                                                               O_BG);
                Eigen::Matrix3d dq_dbg = frame_j->second.pre_integration->jacobian.block<3, 3>(O_R,
                                                                                               O_BG);
                Eigen::Matrix3d dv_dba = frame_j->second.pre_integration->jacobian.block<3, 3>(O_V,
                                                                                               O_BA);
                Eigen::Matrix3d dv_dbg = frame_j->second.pre_integration->jacobian.block<3, 3>(O_V,
                                                                                               O_BG);
                ...
                tmp_b.block<3, 1>(6, 0) = (frame_j->second.pre_integration->delta_q.inverse() *
                                           Eigen::Quaterniond(frame_i->second.R.transpose() *
                                                              frame_j->second.R)).vec();

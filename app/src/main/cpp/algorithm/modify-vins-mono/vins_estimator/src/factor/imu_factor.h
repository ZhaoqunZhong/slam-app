...
namespace Initializer {
    ...
    public:
        ...
        IMUFactor(std::shared_ptr<IntegrationBase> _pre_integration) : pre_integration(
                _pre_integration) {
        }
...
/*        static uint count = 0;
        LOG(INFO) << count % WINDOW_SIZE << " imu factor residual \n"
                << residual.segment<3>(0).transpose() << " \n"
                << residual.segment<3>(3).transpose() << " \n"
                << residual.segment<3>(6).transpose() << " \n"
                << residual.segment<3>(9).transpose() << " \n"
                << residual.segment<3>(12).transpose() << " \n";
        LOG(INFO) << count % WINDOW_SIZE << " imu factor sqrt_info \n" << sqrt_info;
        count++;*/
            ...
            if (jacobians) {
                ...
                if (jacobians[0]) {
                    ...
                    Eigen::Quaterniond corrected_delta_q =
                            pre_integration->delta_q * Utility::deltaQ(
                                    dq_dbg * (Bgi - pre_integration->linearized_bg));
                ...
                if (jacobians[1]) {
                    ...
                    jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = -Utility::Qleft(
                            Qj.inverse() * Qi *
                            pre_integration->delta_q).bottomRightCorner<3, 3>() *
                                                                        dq_dbg;
#endif

                    jacobian_speedbias_i.block<3, 3>(O_V,
                                                     O_V - O_V) = -Qi.inverse().toRotationMatrix();
                    jacobian_speedbias_i.block<3, 3>(O_V, O_BA - O_V) = -dv_dba;
                    jacobian_speedbias_i.block<3, 3>(O_V, O_BG - O_V) = -dv_dbg;

                    jacobian_speedbias_i.block<3, 3>(O_BA,
                                                     O_BA - O_V) = -Eigen::Matrix3d::Identity();

                    jacobian_speedbias_i.block<3, 3>(O_BG,
                                                     O_BG - O_V) = -Eigen::Matrix3d::Identity();
                ...
                if (jacobians[3]) {
                    ...
                    jacobian_speedbias_j.block<3, 3>(O_V,
                                                     O_V - O_V) = Qi.inverse().toRotationMatrix();

                    jacobian_speedbias_j.block<3, 3>(O_BA,
                                                     O_BA - O_V) = Eigen::Matrix3d::Identity();

                    jacobian_speedbias_j.block<3, 3>(O_BG,
                                                     O_BG - O_V) = Eigen::Matrix3d::Identity();

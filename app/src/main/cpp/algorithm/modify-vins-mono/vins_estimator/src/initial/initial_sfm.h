...
namespace Initializer {
	...
	public:
		...
		bool
		solveFrameByPnP(Matrix3d &R_initial, Vector3d &P_initial, int i, vector<SFMFeature> &sfm_f);

		void
		triangulatePoint(Eigen::Matrix<double, 3, 4> &Pose0, Eigen::Matrix<double, 3, 4> &Pose1,
	...
	public:
		...
		Evaluate(double const *const *parameters, double *residuals, double **jacobians) const {
			...
			Eigen::Quaterniond q_cw(parameters[0][6], parameters[0][3], parameters[0][4],
									parameters[0][5]);
			...
			if (jacobians) {
				...
				if (jacobians[1]) {
					Eigen::Map<Eigen::Matrix<double, 2, 3, Eigen::RowMajor>> jacobian_pt(
							jacobians[1]);
